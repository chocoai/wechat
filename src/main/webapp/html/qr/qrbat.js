(function() {
    ...
    qrbtn.addEventListener('click', function(ev) {
        ...
        if(limit) {
            napi = 'http://'+ location.host +'/wx/qrticket?sceneid='+sid+'&limit=10&access_token='+access_token;
        }else{
            napi = 'http://'+ location.host +'/wx/qrticket?sceneid='+sid+'&access_token='+access_token;
        }
        var requestParam = {"sceneid" : sid}
		vge.ajxpost(vge.japi+'/wctAccessToken/getQrCode.do',JSON.stringify(requestParam),50000, function(r){
			qrimg.src=vxapi+r;
